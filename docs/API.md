...
스트릭은 오늘부터 연속된 기록 일수를 계산합니다. 첫 기록을 저장하면 `1`이며, 오늘 기록이 없다면 `0`입니다.
...
- 이미 완료한 미션을 다시 완료하면 `MISSION_ALREADY_COMPLETED`(409)를 반환합니다.
...
- 오늘 미션을 이미 완료한 경우 모든 퀵리플라이 요청은 `MISSION_ALREADY_COMPLETED`(409)를 반환합니다.
